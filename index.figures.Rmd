...
#########Fill Figure
##Year Params Groups
p1 <- viz(munge, factor(munge$Year.Mixed), munge$Parameter.Grp, 'fill', 'Modelled Parameters') +
...
p2 <- viz(filt, filt$Year.Mixed, filt$Model, 'fill', 'Modelling Approach')

## Year Coverage Fill
cov <- filt %>% filter(Coverage.KM != 'unknown')
p3 <- viz(cov, cov$Year.Mixed, cov$Coverage.KM, 'fill', expression(paste('Coverage Scale (km'^'2',')')))
...
# g <- plot_grid(p1,p2,p3,p4, labels = 'AUTO', label_fontface = 'plain', label_size = 12, ncol = 2)
# 
...
##Year Modelling Approach Groups
p2 <- viz(filt, as.factor(filt$Year.Mixed), filt$Model, 'stack', 'Modelling Approach')  +
  scale_fill_brewer(breaks = levels(munge$Model),
                     palette = 'Paired')

## Year Coverage Fill
cov <- filt %>% filter(Coverage.KM != 'unknown')
p3 <- viz(cov, as.factor(cov$Year.Mixed), cov$Coverage.KM, 'stack', expression(paste('Coverage Scale (km'^'2',')')))

## Year Years Fill
p4 <- viz(filt, as.factor(filt$Year.Mixed), filt$Time.Bin, 'stack', 'Study Duration')
...
ggsave(filename='figures/YearDistStackYearMixed.png',plot = g, width=6.5,height=6, units='in', dpi=250)
# 
# g <- plot_grid(p1,p2,p3,p4, labels = 'AUTO', label_fontface = 'plain', label_size = 12, ncol = 2) +
#   draw_text('Year', x = 0.5, y = -.05) +
#   draw_text('Distribution', x = -.05, y = .5) #+
#   theme(plot.margin = margin(b = .25, l = .25, unit = 'in'))
# ?draw_text
# ?margin
# save_plot('figures/YearDistBarStackv3.png', g, base_width =  6.5, base_height = 6)

#############Bar Figure
##Year Params Groups
p1 <- viz(munge, as.factor(munge$Year.Mixed), munge$Parameter.Grp, 'bar', 'Modelled Parameters') +
  scale_fill_brewer(breaks = levels(munge$Parameter.Grp), labels = c("Algae", "Carbon", "Clarity", "Suspended\nSediment", "Turbidity", "Nutrients", "Other"),
                     palette = 'Paired')

##Year Modelling Approach Groups
p2 <- viz(filt, as.factor(filt$Year.Mixed), filt$Model, 'bar', 'Modelling Approach')  +
  scale_fill_brewer(breaks = levels(munge$Model),
                     palette = 'Paired')
...
save_plot('figures/YearDistBarv3.png', g, base_width =  6.5, base_height = 6)
...
ggsave(filename='figures/YearWaterbody2Panelv2.png',plot = g, width=6,height=2.8, units='in', dpi=250)
...
######V3 (Inputs generated in bibliometrix_Report.Rmd)
...
ggsave(filename = 'figures/CountryJournalv2.png', plot = g, height = 6, width = 3.5, unit = 'in', dpi = 250)
...
ggplot() + 
  ...
  scale_y_continuous(limits = c(0,125), expand =c(0,0),
                     sec.axis = sec_axis(~.*13, name  = 'Average Citation Count')) + theme_bw() +
...
ggsave('figures/PubCountsYearlyv2.png', width = 4, height = 4, unit = 'in', dpi = 300)
...
ggsave(filename='figures/Spatialv3.png',plot = g, width=4,height=7, units='in', dpi=250)
...
ggplot(df, aes(x = Cat, y = fig.count/fig.total)) + geom_violin(aes(colour = fig.type)) + theme_bw() + labs(x = 'Paper Category', y = 'Figure Proportion')
...
ggplot(df, aes(x = Cat, y = fig.count/fig.total)) + geom_col(aes(fill = fig.type)) + theme_bw()
...
ggsave('figures/R2.png', width = 4, height = 3, units = 'in')
