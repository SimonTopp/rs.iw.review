...
```{r}
##Error metric figure playing
grepl(x = index$EF.metric, pattern = 'rmse',  ignore.case = T)

## Super annoying way to make long error data frame
ef <- index.munge %>%
  mutate(model.id = row_number()) %>%
  select(ID, model.id,Year, Parameter, Parameter.Grp, Error.fit1:Error.fit4) %>%
  gather(Error.fit1:Error.fit4, key = fit.num, value = Error.fit)
ef$fit[ef$fit.num == 'Error.fit1'] <- 1
ef$fit[ef$fit.num == 'Error.fit2'] <- 2
ef$fit[ef$fit.num == 'Error.fit3'] <- 3
ef$fit[ef$fit.num == 'Error.fit4'] <- 4
ef <- ef %>% mutate(UniqueID = paste0(model.id,'.', fit))
 
efm <- index.munge %>%
  mutate(model.id = row_number()) %>%
  select(ID, model.id,Year, Parameter, Parameter.Grp, Fit.metric1:Fit.metric4) %>%
  gather(Fit.metric1:Fit.metric4, key = fit.num, value = Fit.metric)
efm$fit.metric[efm$fit.num == 'Fit.metric1'] <- 1
efm$fit.metric[efm$fit.num == 'Fit.metric2'] <- 2
efm$fit.metric[efm$fit.num == 'Fit.metric3'] <- 3
efm$fit.metric[efm$fit.num == 'Fit.metric4'] <- 4
efm <- efm %>% mutate(UniqueID = paste0(model.id, '.',fit.metric)) 

error.fit <- left_join(ef, efm %>% select(Fit.metric, UniqueID), by = 'UniqueID')

ev <- index.munge %>%
  mutate(model.id = row_number()) %>%
  select(ID, model.id,Year, Parameter, Parameter.Grp, Error.val1:Error.val4) %>%
  gather(Error.val1:Error.val4, key = val.num, value = Error.val)
ev$val[ev$val.num == 'Error.val1'] <- 1
ev$val[ev$val.num == 'Error.val2'] <- 2
ev$val[ev$val.num == 'Error.val3'] <- 3
ev$val[ev$val.num == 'Error.val4'] <- 4
ev <- ev %>% mutate(UniqueID = paste0(model.id,'.', val))
 
evm <- index.munge %>%
  mutate(model.id = row_number()) %>%
  select(ID, model.id,Year, Parameter, Parameter.Grp, Val.metric1:Val.metric4) %>%
  gather(Val.metric1:Val.metric4, key = val.num, value = Val.metric)
evm$val.metric[evm$val.num == 'Val.metric1'] <- 1
evm$val.metric[evm$val.num == 'Val.metric2'] <- 2
evm$val.metric[evm$val.num == 'Val.metric3'] <- 3
evm$val.metric[evm$val.num == 'Val.metric4'] <- 4
evm <- evm %>% mutate(UniqueID = paste0(model.id, '.',val.metric)) 

error.val <- left_join(ev, evm %>% select(Val.metric, UniqueID), by = 'UniqueID')

error.long <- error.val %>% left_join(error.fit) %>%
  mutate(Fit.metric = trimws(Fit.metric),
         Val.metric = trimws(Val.metric)) #%>%
  gather(Error.fit, Error.val, key = 'Type', value = 'Error' ) %>%
  select(ID, model.id, Parameter, Parameter.Grp, val, Type, Error)

error.long$Fit.metric[error.long$Fit.metric == ''] = 'none'
error.long$Val.metric[error.long$Val.metric == ''] = 'none'


## Do some munging
### Look at the unique values in order
unique(error.long$Fit.metric)[order(unique(error.long$Fit.metric))]
...
error.long <- error.long %>% mutate(Fit.metric = ifelse(grepl(pattern = terms[[i]], Fit.metric), names(terms[i]), as.character(Fit.metric)))
}
...
### R2 Values
r <- error.long %>% filter(Fit.metric == 'R2')%>%
  mutate(Error.fit = as.numeric(Error.fit)) %>%
  na.omit() %>%
  filter(Error.fit > 0.1)

ggplot(r, aes(x = Year, y = Error, color = Type))+ geom_smooth()
```
